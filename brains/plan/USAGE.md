...
```
~/.claude/skills/apb/brains/plan/
```

Do not copy brain files into your project. The brain reads your project directory — it lives in the skill pack.
...
The brain runs: Premortem, Inversion, Bottleneck/Mitigation, Optionality. After each tool, it shows you what it found and waits for your acknowledgment before continuing.
...
You can name any combination: `premortem`, `inversion`, `bottleneck`, `optionality`. The brain still requires the premortem to be completed (now or previously) before it can mark /plan as DONE.
...
| CLAUDE.md | Brain folder (not your project) | Already exists |
...
/plan takes the validated problem from /discover and stress-tests the direction before any infrastructure, requirements, or code work begins. Every downstream brain can reference the risks and guardrails from /plan.
