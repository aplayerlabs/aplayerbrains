...
## WD-002: Continuation prompt points to a brain, not just "resume"
**Date:** 2026-04-06
**Decision:** The continuation prompt explicitly names the target brain and mode, rather than saying "resume where you left off."
**Why:** The next session is a blank slate. It doesn't know what "resume" means. By naming the brain, the mode, and the exact stopping point, the continuation prompt gives the new session everything it needs to start working in under 30 seconds. It also handles backward movement — /test wrapping to /build FIX — which a generic "resume" prompt can't express.
