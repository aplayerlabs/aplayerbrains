...
Do not copy these files into your project. The skill system reads them from the brains directory.
...
Triggered automatically when context is running low. You don't invoke this directly — the active brain triggers it when it detects context filling up.
...
Read @brains/build/CLAUDE.md, then work on @cc/clients/Advance/Dashboard/
...
/wrap works with ANY brain. It reads which brain is active from SESH.md, looks up that brain's modes, and generates the right continuation prompt.

The continuation prompt can point to a different brain than the one you were using:

- **/test found bugs?** Continuation points to `/build FIX`
- **/build fixed the bugs?** Continuation points to `/test VERIFY`
- **Brain's work is done?** Continuation points to the next brain in the pipeline
