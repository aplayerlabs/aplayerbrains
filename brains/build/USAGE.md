...
This brain lives in the A Player Brains skill pack:

```
~/.claude/skills/apb/brains/build/
```
...
If no project structure exists, /build automatically enters BOOTSTRAP mode. It reads the PRD, picks the stack, scaffolds the project, creates Stage 0 tasks, and sets you up to build.
...
|----------|----------|---------------------|
| `~/.claude/skills/apb/brains/build/` | The brain's operating contract | Never — this is the machine |
| `~/your-project/` | App code, tasks, bugs, docs, SESH.md, STATUS.md | Yes — this is the deliverable |

---

## What /build Creates (BOOTSTRAP)

On the first session, /build creates the full project skeleton:
...
SESH.md and STATUS.md already exist from earlier brains (or /build creates them if entering directly).

---

## How It Works With Other Brains

### Normal pipeline flow

```
/design finishes → business owner approved visual direction
/build BOOTSTRAP → scaffolds project, creates tasks
...
|------|------|-------------|
| BOOTSTRAP | First session | Picks stack, scaffolds project, creates tasks |
