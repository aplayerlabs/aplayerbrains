...
**Why:** MOCKUP and DESIGN have different toolchains (HTML files vs. Figma Scripter). REFINE works on either path and has different behavior — it reads existing work and iterates rather than generating from scratch. Mixing "create" and "iterate" in the same mode risks the brain regenerating everything when the business owner just wants to tweak a header color. Explicit modes make the intent clear.
