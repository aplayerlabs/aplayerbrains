...
Turn the business owner's validated direction into a sharp product requirements document (PRD) that downstream brains can execute against without drift.
...
**Opening (the brain proposes, not asks):**
...
The brain proposes the flow based on everything learned:
...
**Complete when:** User confirms the flow. The brain notes extrapolated UX states internally for the PRD.
...
**Refusals state:** what is missing, which phase or rule is incomplete, which brain handles it.
